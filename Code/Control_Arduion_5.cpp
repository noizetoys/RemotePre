...
void updateDisplay() {
    // Update OLED display with current property states
    display.clearDisplay();
    ...
    display.print("Mute: ");
    display.println(prev_mute_state ? "On" : "Off");
    display.display();
}
...
void sendVolumeUpdate(uint16_t volume) {
    // Function to send volume update over I2C
    // Implement according to your requirements
}

void toggleMute() {
    // Function to toggle mute state
    prev_mute_state = !prev_mute_state;
    // Implement any action needed when mute state changes
}
